...
type Section = 'feed' | 'questions' | 'faqs'

interface Post {
  id: number; username: string; country: string; initials: string
  avatarBg: string; img: string; imgH: number; text: string; tags: string[]; likes: number
}
interface Question {
  id: number; username: string; country?: string; initials: string
  avatarBg?: string; anonymous: boolean; time: string; text: string; answers: number; helpful: number
}
...
const SEED_POSTS: Post[] = [
  { id: 1, username: 'Mei Lin', country: 'China', initials: 'ML', avatarBg: 'linear-gradient(135deg, #FFD6B0, #FF9A3C)', img: 'https://picsum.photos/seed/harvard1/400/320', imgH: 200, text: "Just arrived at Harvard! The campus is absolutely beautiful. Can't wait to explore everything.", tags: ['arrival', 'harvard', 'excited'], likes: 42 },
  { id: 2, username: 'Lucas M.', country: 'Brazil', initials: 'LM', avatarBg: 'linear-gradient(135deg, #B8FFD0, #3CB87A)', img: 'https://picsum.photos/seed/coffee2/400/420', imgH: 260, text: 'Best coffee on campus is at the Science Center café. Trust me on this one.', tags: ['tips', 'food', 'studylife'], likes: 87 },
  { id: 3, username: 'Priya S.', country: 'India', initials: 'PS', avatarBg: 'linear-gradient(135deg, #B8D8FF, #5599EE)', img: 'https://picsum.photos/seed/student3/400/290', imgH: 180, text: 'Got my student ID and bank account sorted in one day! DM me if you need help navigating the admin stuff.', tags: ['admin', 'banking', 'studentid'], likes: 134 },
  { id: 4, username: 'Ji-ho P.', country: 'South Korea', initials: 'JP', avatarBg: 'linear-gradient(135deg, #F0C8FF, #CC66FF)', img: 'https://picsum.photos/seed/campus4/400/370', imgH: 230, text: 'The scavenger hunt was incredible. Our team found every hidden spot on campus!', tags: ['scavengerhunt', 'team', 'campus'], likes: 56 },
  { id: 5, username: 'Omar K.', country: 'Egypt', initials: 'OK', avatarBg: 'linear-gradient(135deg, #FFE0B0, #FF8C00)', img: 'https://picsum.photos/seed/yard5/400/340', imgH: 210, text: 'Harvard Yard in the morning light is something else entirely. 6am walks are now a ritual.', tags: ['harvardyard', 'morning', 'vibes'], likes: 203 },
  { id: 6, username: 'Sofia R.', country: 'Germany', initials: 'SR', avatarBg: 'linear-gradient(135deg, #FFB8C8, #EE4466)', img: 'https://picsum.photos/seed/group6/400/270', imgH: 170, text: 'Found the European Students group — so nice to meet everyone from home regions!', tags: ['europeans', 'community', 'friends'], likes: 78 },
]
...
const SEED_FAQS: FAQ[] = [
  ...
  { id: 2, q: 'How do I get my Harvard ID?', a: 'Visit the Harvard ID Services office in Smith Campus Center (1350 Massachusetts Ave) with your passport and admission letter. Usually takes 15 minutes.' },
  ...
  { id: 4, q: 'Can I work on campus as an international student?', a: 'F-1 visa holders may work on-campus up to 20 hours/week during the semester. Check with the Harvard International Office (HIO) for eligibility.' },
  { id: 5, q: 'What health insurance do I need?', a: 'Harvard requires all students to have health insurance. Most international students are auto-enrolled in the Harvard Student Health Insurance Plan (SHIP).' },
  { id: 6, q: 'How do I get a US phone number?', a: 'T-Mobile and AT&T offer prepaid SIM cards without a US credit history. Google Fi is popular with international students for its international features.' },
]

// ─── Section picker config ────────────────────────────────────────────────────

const SECTIONS: { id: Section; label: string; sub: string; iconBg: string; icon: React.ReactNode }[] = [
  {
    id: 'faqs',
    label: 'Common Questions',
    sub: 'FAQs for international students',
    iconBg: 'linear-gradient(135deg, #FFF0E0, #FFD6A0)',
    icon: <PinIcon size={20} color="#FF9A3C" />,
  },
  {
    id: 'questions',
    label: 'Community Questions',
    sub: 'Ask & answer the community',
    iconBg: 'linear-gradient(135deg, #E8EFFF, #C8D8FF)',
    icon: <ChatIcon size={20} color="#5599EE" />,
  },
  {
    id: 'feed',
    label: 'Community Feed',
    sub: 'Photos & stories from students',
    iconBg: 'linear-gradient(135deg, #E0FFF0, #A0F0C0)',
    icon: <HeartIcon size={20} color="#3CB87A" />,
  },
]
      ...
      <div style={{ padding: '14px 16px 12px' }}>
        ...
        {post.tags.length > 0 && (
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 12 }}>
            {post.tags.map(tag => <span key={tag} style={{ fontSize: 12, color: 'var(--orange)', fontWeight: 600 }}>#{tag}</span>)}
          </div>
        )}
    ...
    <div style={{ background: '#fff', borderRadius: 16, padding: '16px', boxShadow: '0 2px 10px rgba(0,0,0,0.05)', marginBottom: 12 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
        ...
        <div>
          ...
          <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>{q.country ? `${q.country} · ` : ''}{q.time}</div>
        </div>
      </div>
...
function FAQCard({ faq }: { faq: FAQ }) {
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 16, padding: '16px', boxShadow: '0 2px 10px rgba(0,0,0,0.05)', marginBottom: 10 }}>
      ...
      {open && (
        <p style={{ fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.6, marginTop: 12, borderTop: '1px solid var(--border)', paddingTop: 12 }}>{faq.a}</p>
      )}
    </div>
  )
}
...
function CreatePostSheet({ onClose, onSubmit }: { onClose: () => void; onSubmit: (p: Post) => void }) {
  ...
  const handleSubmit = () => {
    if (!text.trim()) return
    const seed = Date.now()
    onSubmit({ id: seed, username: 'You', country: 'Your Country', initials: 'ME', avatarBg: 'linear-gradient(135deg, #FFC94A, #FF9A3C)', img: `https://picsum.photos/seed/${seed}/400/320`, imgH: 200, text: text.trim(), tags: tags.split(' ').filter(t => t.startsWith('#')).map(t => t.slice(1)), likes: 0 })
  }
  return (
    <div onClick={(e) => e.target === e.currentTarget && onClose()}
      ...
      <div className="slide-up" style={{ background: '#fff', borderRadius: '24px 24px 0 0', width: '100%', maxWidth: 430, padding: 24, paddingBottom: 40 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
          ...
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', padding: 4 }}><CloseIcon size={20} color="#4A4A4A" /></button>
        </div>
        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12, marginBottom: 16 }}>
          <Avatar name="Me" size={38} />
          <textarea value={text} onChange={e => setText(e.target.value)} placeholder="Share something with the community..." autoFocus
        ...
        <div style={{ border: '1.5px solid var(--border)', borderRadius: 12, padding: '10px 14px', marginBottom: 16 }}>
          <div style={{ display: 'flex', gap: 12 }}>
            <button style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', cursor: 'pointer', fontSize: 13, color: 'var(--text-secondary)', padding: 0 }}><CameraIcon size={18} color="var(--text-secondary)" /> Photo</button>
            <button style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', cursor: 'pointer', fontSize: 13, color: 'var(--text-secondary)', padding: 0 }}><VideoIcon size={18} color="var(--text-secondary)" /> Video</button>
            <button style={{ display: 'flex', alignItems: 'center', gap: 6, background: 'none', border: 'none', cursor: 'pointer', fontSize: 13, color: 'var(--text-secondary)', padding: 0 }}><PinIcon size={18} color="var(--text-secondary)" /> Location</button>
          </div>
        </div>
        <input value={tags} onChange={e => setTags(e.target.value)} placeholder="Add tags, e.g. #harvard #tips #arrival"
        ...
        <button onClick={handleSubmit}
          style={{ width: '100%', background: 'linear-gradient(135deg, #FFC94A, #FF9A3C)', border: 'none', borderRadius: 14, padding: '14px 0', fontSize: 16, fontWeight: 700, color: '#fff', cursor: 'pointer', boxShadow: '0 4px 16px rgba(255,154,60,0.35)' }}>Post</button>
      </div>
    </div>
  )
}
  ...
  return (
    <div onClick={(e) => e.target === e.currentTarget && onClose()}
      ...
      <div className="slide-up" style={{ background: '#fff', borderRadius: '24px 24px 0 0', width: '100%', maxWidth: 430, padding: 24, paddingBottom: 40 }}>
        ...
        <textarea value={text} onChange={e => setText(e.target.value)} placeholder="What would you like to ask?" autoFocus
        ...
        <button style={{ width: '100%', background: text.trim() ? 'linear-gradient(135deg, #FFC94A, #FF9A3C)' : '#E8E8E8', border: 'none', borderRadius: 14, padding: '14px 0', fontSize: 16, fontWeight: 700, color: text.trim() ? '#fff' : '#9A9A9A', cursor: text.trim() ? 'pointer' : 'default', transition: 'all 0.2s' }}
          onClick={() => text.trim() && onClose()}>Ask</button>
      </div>
    </div>
  )
}
...
function SectionPickerSheet({ current, onSelect, onClose }: { current: Section; onSelect: (s: Section) => void; onClose: () => void }) {
  return (
    <div onClick={(e) => e.target === e.currentTarget && onClose()}
      style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.35)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', zIndex: 200 }}>
      <div className="slide-up" style={{ background: '#fff', borderRadius: '24px 24px 0 0', width: '100%', maxWidth: 430, paddingBottom: 40, overflow: 'hidden' }}>
        {/* Handle */}
        ...
        {SECTIONS.map((s, i) => (
          <button key={s.id} onClick={() => { onSelect(s.id); onClose() }}
            style={{
              ...
              width: '100%', background: current === s.id ? '#FAFAFA' : '#fff',
              border: 'none', borderTop: i > 0 ? '1px solid #F0F0F0' : 'none',
              padding: '16px 24px', cursor: 'pointer', textAlign: 'left',
            }}>
            {/* Icon circle */}
            ...
            {/* Active dot */}
            {current === s.id && (
              <div style={{ width: 8, height: 8, borderRadius: '50%', background: '#5599EE', flexShrink: 0 }} />
            )}
          </button>
        ))}
      </div>
...
export default function Home() {
  ...
  const filteredQuestions = SEED_QUESTIONS.filter(q => !search || q.text.toLowerCase().includes(search.toLowerCase()))
    ...
    <div className="fade-in" style={{ background: 'var(--bg)', minHeight: '100%' }}>
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid var(--border)', padding: '12px 20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <button onClick={() => setShowPicker(true)}
            ...
            <div style={{ width: 32, height: 32, borderRadius: '50%', background: currentSection.iconBg, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              {currentSection.icon}
            </div>
            <div style={{ textAlign: 'left' }}>
              <div style={{ fontSize: 16, fontWeight: 800, color: 'var(--text)' }}>{currentSection.label}</div>
              <div style={{ fontSize: 12, color: 'var(--text-secondary)' }}>{currentSection.sub}</div>
            </div>
            {/* chevron */}
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--text-secondary)" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round" style={{ marginLeft: 2 }}>
              <polyline points="6 9 12 15 18 9" />
            </svg>
          </button>
      ...
      {section === 'feed' && (
        <>
          ...
          {showCreate && <CreatePostSheet onClose={() => setShowCreate(false)} onSubmit={p => { setPosts(prev => [p, ...prev]); setShowCreate(false) }} />}
        </>
      )}
      ...
      {section === 'faqs' && (
        <div style={{ padding: '16px 16px 100px' }}>
          <p style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 16 }}>Tap a question to expand the answer.</p>
      ...
      {showPicker && (
        <SectionPickerSheet current={section} onSelect={s => setSection(s)} onClose={() => setShowPicker(false)} />
      )}
    </div>
  )
}
